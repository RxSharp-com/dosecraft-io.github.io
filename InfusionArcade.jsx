...
export default function InfusionArcade() {
  ...
  useEffect(() => {
    ...
    function tickBreakout(s) {
      ...
      if (s.bricks.every(b => !b.alive)) { cancelAnimationFrame(animRef.current); setScreen("win"); return false; }
    ...
    function tickVanco(s) {
      ...
      tickP(s);
      if (s.total >= 48 && s.precursors.every(p => p.bound || p.missed || p.opacity <= 0)) { cancelAnimationFrame(animRef.current); setScreen("win"); return false; }
    ...
    function advanceDaptoWave(s) {
      s.wave++; if (s.wave >= TOTAL_WAVES) { cancelAnimationFrame(animRef.current); setScreen("win"); return false; }
    ...
    function tickIvig(s) {
      ...
      if (s.ivigWave === 1) {
        ...
        if (occupiedCount / s.totalReceptors >= 0.7) {
          cancelAnimationFrame(animRef.current); setScreen("win"); return false;
        }
  ...
  if (screen === "intro") return (
    <div style={pageStyle}>
      <div style={{ maxWidth: 460, width: "100%" }}>
        ...
        <div style={{ background: col + "18", border: `1.5px solid ${col}55`, borderRadius: 12, padding: "14px 18px", marginBottom: 28, textAlign: "center" }}>
          <p style={{ fontSize: 15, color: col, fontWeight: 600, margin: 0, lineHeight: 1.6 }}>✦ {drug.encouragement}</p>
        </div>
        <div style={{ display: "flex", gap: 12, justifyContent: "center", flexWrap: "wrap" }}>
          <BigBtn label={`Watch ${drug.name} work →`} onClick={() => setScreen("playing")} primary />
    ...
    <div style={pageStyle}>
      <div style={{ maxWidth: 440, width: "100%", textAlign: "center" }}>
        ...
        <div style={{ display: "flex", gap: 12, justifyContent: "center", flexWrap: "wrap" }}>
          ...
          <BigBtn label="Choose another medication" onClick={() => setScreen("menu")} />
        </div>
        <SharpRXBadge />
      </div>
    </div>
  ...
  return (
    <div style={{ ...pageStyle, padding: 8 }}>
      ...
      <div style={{ marginTop: 12, display: "flex", gap: 10, alignItems: "center", justifyContent: "center" }}>
        <button onClick={() => { cancelAnimationFrame(animRef.current); setScreen("menu"); }} style={{ background: "rgba(255,255,255,0.1)", border: "1px solid rgba(255,255,255,0.2)", color: "rgba(255,255,255,0.7)", padding: "10px 20px", borderRadius: 8, fontSize: 13, cursor: "pointer", fontFamily: SANS, touchAction: "manipulation" }}>← Menu</button>
